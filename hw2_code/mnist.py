...
odds = [1,3,5,7,9]
evens = [2,4,6,8,0]
pairwise_classifications = [[1,7],[3,5],[4,9], [odds, evens]]
C = [10**(-i) for i in xrange(6)]
bands = [0.1,1,4]
...
def CSVM_MNIST_pair(pair):
	...
	for c in C:
		...
		b = 1.0/num_marginal*(np.sum(Ys[marginal]-np.apply_along_axis(sum_kf, 1, Xs[marginal[0]])))
	...
	test_results = np.sign(best_b + np.dot(best_w, np.transpose(np.concatenate((neg_test, pos_test)))))
...
def RBF_MNIST_pair(pair):
	...
	accuracy = (test_results == test_Y).sum()/float(test_results.shape[0])
	print("Test Accuracy: ", accuracy)
